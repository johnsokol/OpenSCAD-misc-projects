height = 40; width = 150; depth = 370;
thickness = 4.2; cornerRadius = 20; tabLength = 10;
  ...
  if(flat)
  {
    union() 
    {
     ...
     translate([0,0,  (height+thickness)/2])
      ...
      translate([depth + 20,0,  (height+thickness)/2])
        boxLid(height, width, depth, thickness, cornerRadius);
 
    rotate([0,0,90])
      translate([120,0,(height+thickness)/2])     
...
module makeTabs(noTabs, tabLength, panelThickness)
{
  union()
    ...
    color("red")
        cube([tabLength, panelThickness+0.1, panelThickness], true);
}
